import css from "../shell/shell.min.css";

export function shell() {
    // global variables declare
    window.scroll = function() {
        $('#windowShellContent').scrollTop($('#windowShellContent')[0].scrollHeight);
    }
    window.halfscroll = function() {
        $('#windowShellContent').scrollTop($('#windowShellContent').scrollTop() + 75);
    }
    ...
    //shell framework credit: https://stackoverflow.com/a/64217174
    var history = document.getElementById('shellHistory');
    var input = document.getElementById('shellInput');
    var cursor = document.getElementById('shellCaret');

    // function to append content to shell with blockquote
    window.appendShell = function(data, id) {
    ...
    window.appendShellError = function(data, example) {
        if (example) {
    ...
    window.appendShellNoAuth = function() {
        appendShellError("user not authenticated, please login or register first to access this function.")
    }
    ...
    function handleCommand(originalVal) {
        ...
        switch (inputVal[0]) {
            case "":
                ...
                break;
            case "help":
                appendShell('ACCESSING HELP MODULE...')
                ...
                break;
            case "cmatrix":
                appendShell('ACCESSING CMATRIX MODULE...')
            ...
            case "sdc":
                ...
                if (inputVal[1]) {
                    appendShell('ACCESSING SDC CONSOLE...')
                    if (inputVal[1] == "start") {
                        appendShell('ACCESSING SDC KERNEL...')
                        import("../shell/command/hacking-mode/sdc/sdc.js").then(function(module) {
                            window.sdc = module.sdc
                            sdc()
                        })
                    } else if (inputVal[1] == "install") {
                        appendShellError("SDC assets are already installed on the mainframe")
                    } else {
                        appendShellError(`invalid argument : ${inputVal[1]}`)
                    }
                } else {
                    appendShellError("please enter an argument or a command", "Example: sdc <b>start</b>")
                }
                break;
            case "access":
                window.pageContent = null
                if (inputVal[1]) {
                    appendShell('ACCESSING DOCUMENT...')
                    var clearance = 0;
                    if (userState) {
                    ...
                    if (inputVal[2] && isInt(inputVal[2]) && userState && inputVal[2] <= Number(userInfo.clearance)) {
                        ...
                        appendShellError('invalid or insufficient security clearance level, attempting to display the Lv. 0 clearance version of the file')
                    }
                    import('../shell/command/access/access.js').then(module => {
                        window.access = module.access
                        access(inputVal[1], clearance)
                    })
                } else {
                    appendShellError("please enter the filename/scp number of the document you wish to access", "Example: ACCESS <u>173</u>")
                }
                break;
            case "auth":
                appendShell('ACCESSING AUTHENTICATION MODULE...')
                import('../shell/command/auth/auth.js').then(module => {
                    window.auth = module.auth
                    auth()
                })
                break;
            case "lockout":
                appendShell('ACCESSING LOCKOUT MODULE...')
                import('../shell/command/lockout/lockout.js').then(module => {
                    window.lockout = module.lockout
                    lockout()
                })
                break;
            case "locate":
                appendShell('ACCESSING LOCATION MODULE...')
                import('../shell/command/locate/locate.js').then(module => {
            ...
            case "clear":
                ...
                break;
            case "fullscreen":
                import('../shell/command/fullscreen/fullscreen.js').then(module => {
            ...
            case "fullquit":
                ...
                break;
            case "whoami":
                appendShell('ACCESSING USER PROFILE MODULE...')
                import('../shell/command/profile/whoami/whoami.js').then(module => {
                    window.whoami = module.whoami
                    whoami()
                })
                break;
            case "bgm":
                appendShell('ACCESSING BACKGROUND MUSIC/AUDIO MODULE...')
                import('../shell/command/bgm/bgm.js').then(module => {
                    window.bgm = module.bgm
                    bgm()
                })
                break;
            default:
                appendShellError(`command not found: ${inputVal[0]}`)
                addLog(`command not found: ${inputVal[0]}`)
        }
    }
    document.addEventListener('selectionchange', () => {
        if (document.activeElement.id !== 'shellInput') return;
        ...
        if (end < length) {
            ...
            $("#shellInput").css("caret-color", "var(--text)")
        } else {
            $("#shellInput").css("caret-color", "")
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (String.fromCharCode(e.keyCode).match(/(\w|\s)/g) && ctrl) {
            if (!$("input:not('#shellInput')").is(":focus")) {
                if (e.target !== input) focusAndMoveCursorToTheEnd();
                scroll()
            }
        }
    });
    $(document).on("click", "#shellInputArea", function() {
        input.focus();
        scroll();
    })
    $(document).on("click", "#shellCaret", function() {
